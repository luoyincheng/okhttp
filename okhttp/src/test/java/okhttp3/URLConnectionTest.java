...
public final class URLConnectionTest {
	...
	@BeforeEach
	public void setUp(MockWebServer server, MockWebServer server2) {
		this.server = server;
	...
	@Test
	public void response307WithPostReverted() throws Exception {
		client = client.newBuilder()
	...
	@Test
	public void response308WithPostReverted() throws Exception {
		client = client.newBuilder()
			.addNetworkInterceptor(new LegacyRedirectInterceptor())
			.build();
	...
	enum ProxyConfig {
		NO_PROXY() {
			@Override
			public Call.Factory connect(MockWebServer server, OkHttpClient client) {
